...
print("--- Generating Task 2 Visualization Assets ---")
...
plt.tight_layout()
plt.savefig('Task-2-EDA/assets/sales_distribution.png')
plt.close()

# 2. Revenue by Product Category (Bar Chart)
plt.figure()
q1_data = df.groupby('Category')['Total_Sales_Value'].sum().sort_values(ascending=False).reset_index()
sns.barplot(x='Total_Sales_Value', y='Category', data=q1_data.head(5), palette='Blues_r')
plt.title('Top 5 Product Categories by Generated Revenue')
plt.xlabel('Total Revenue (INR)')
plt.ylabel('Category')
plt.tight_layout()
plt.savefig('Task-2-EDA/assets/category_revenue.png')
plt.close()

# 3. Top States by Revenue (Bar Chart)
plt.figure()
...
plt.ylabel('State')
plt.tight_layout()
plt.savefig('Task-2-EDA/assets/top_states_revenue.png')
plt.close()

# 4. Shipping Urgency vs Average Order Value (Multivariate Chart)
plt.figure()
...
plt.xlabel('Shipping Priority Class')
plt.ylabel('Total Sales Value (INR)')
plt.tight_layout()
plt.savefig('Task-2-EDA/assets/urgency_vs_value.png')
plt.close()

print("Success! All 4 analytics charts saved inside your 'Task-2-EDA' folder.")
